...
```
ok so now after that you want to make a new folder (update 2.1: now its a zip) now put the main.py in that folder than put your ffmpeg in that folder
```IMPORTANT
...
working stuff is non-placeholder home/new videos feed, explore (not search) and tags and non-placeholder video playback rest is to be implemented maybe in the future.
